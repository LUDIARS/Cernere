-- EducationLab の Cernere project 登録と、共通 EducationPartner profile schema の更新。
-- 個人プロフィールは EducationLab DB に複製せず project_data_vantan_user を単一情報源にする。
...
VALUES (
    'EducationLab',
    'EducationLab',
    'EducationPartner EducationLab operations hub.',
    ...
    '{
        "project": {
            "key": "EducationLab",
            "name": "EducationLab",
            "description": "EducationPartner EducationLab operations hub."
        },
        "endpoint": {
            "url": "http://localhost:5187",
            "frontend_url": "http://localhost:5187",
            "same_server": true,
            "bridge_path": "/EducationLab"
        },
        "data_sharing": [],
        "user_data": { "columns": {} }
    }'::jsonb
...
VALUES (
    'vantan_user',
    'EducationPartner User Profile',
    'EducationPartner 共通プロフィール。氏名・役職・所属学科を必須情報として Cernere に一元保管する。',
    gen_random_uuid()::text,
    crypt(gen_random_uuid()::text, gen_salt('bf', 12)),
    '{
        "project": {
            "key": "vantan_user",
            "name": "EducationPartner User Profile",
            "description": "氏名・役職・所属学科などの EducationPartner 共通プロフィール"
        },
        "data_sharing": [
            {
                "project_key": "aedilis",
            ...
            },
            {
                ...
                "modules": ["profile"],
                "access": "readwrite",
                "description": "EducationLab 初回アクセス時のプロフィール登録と運営表示に使用"
            }
        ],
        "user_data": {
            "columns": {
...
-- 既存の未登録行を保持したまま、必須性は schema と EducationLab 登録フォームで強制する。
