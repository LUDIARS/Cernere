...
- **除く（他プロジェクトからの委託データ）**:
  ...
  - `volputas_survey_responses` / `volputas_survey_answers`もCernere所有テーブルだが、
    Volputas設問に対する本人回答を預かる委託データ。設問定義はVolputasが所有する。
...
| | `project_credential_issuers` | 起動credentialを発行できるlauncher許可 |
| | `project_launch_credentials` | Ex等から受領した起動secretの暗号化履歴 |
