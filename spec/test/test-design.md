...
- **対象と観点**:
  - **トークン層**（実施済）
    - `server/src/auth/jwt.ts` — access / project (HS256) の発行・検証、
    ...
    - `server/src/auth/paseto.ts` — Ed25519 sign/verify、**audience 不一致
      ...
      鍵ロード / `getPublicKeys`。
  - **入力バリデーション**（一部実施）— 各 module request の Zod スキーマ
    （境界値・必須欠落・型不正・enum 外）。`volputas_survey.*`はUUID、
    question ID、100件、4,000文字、U+0000 / unpaired surrogate拒否、int32、
    exactly-one、重複を固定済み。
  - **transactional replace**（実施済）— Volputas再回答のupsert、旧answer削除、
    新answer挿入を同一transactionで行い、挿入失敗時に以前の回答を保持する。
  - **センシティブログ**（実施済）— Volputas response / answer SQLのparameterに
    canaryを入れてもDrizzle / postgres.js dev logへ出ないことを固定する。
...
- **対象と観点**（`server/src/ws/auth.ts` / `server/src/redis.ts`）:
  ...
  - **project command認可** — `volputas_survey.*`は`projectKey=volputas`だけが利用でき、
    別projectはservice呼出し前に拒否される。
...
- `036_volputas_survey_responses.sql`はproject/issuer seed、FK、unique、exactly-one
  CHECK、index、禁止SQL不在をunit testで静的固定する。実PostgreSQL再適用はCI項目。
