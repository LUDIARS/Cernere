...
> **composite ログインは撤廃する** (移行しない)。 host (= **Corpus app**、 §13) が
...
host (= **Corpus app**、 §13 で確定) は Cernere の **常時接続セッション** を握り、
その **user accessToken** を保持し続ける ([[project_cernere]] の Always-Connected
Session + Corpus D5)。 Excubitor は ops plane であり SSO host にしない (§13)。
1 度 host にログインすれば、 以後ユーザがどのサービスを
...
以下の「同設計書」 は
[端末セッション・運用者回復設計](../plan/passkey-default-authentication.md)
を指す。

- **登録済みパスキー一覧** — 登録日時、 最終使用時刻、 backup state。
  現行プロフィールとの統一が必要。#886 の端末一覧は日時と種類を表示する
...
資格情報の変更と失効は fresh step-up を要求する (同設計書 §6–8)。 通常の access token
だけでは実行させない。

#### 復旧経路の扱い

パスキーの回復は**運用者発行の一回限り登録用トークン**による
(同設計書 §7)。 したがって本パネルには:

- **置かない**: 「パスキーを忘れた」 相当のボタン、 回復トークンを要求する導線、
  email/SMS によるセルフサービス回復
- **置く**: 別窓口へ誘導する静的な案内文と、 運用者が account を特定するために
  利用者が提示する opaque `user_id` の表示

三モード分離は採用しない。Cr 本体の管理者用 /account-recovery と回復受領者用 /recover を使う。
この共通 descriptor の案内とは分け、公開利用者から grant を要求する導線は設けない。
...
## 12. Vite 風 UI 配信 (lazy / cache / HMR)

> 2026-06-08 ユーザ確定。 §11 の UI key レジストリを **Vite の module 配信** に
> なぞらえる方針。 「各サービスの Vite フロントを Corpus に集約」 (§1.4) を、
> 配信機構まで Vite-like にする。 **正本は Corpus DESIGN.md §13** へ反映する。

### 12.1 対応表 (Vite → Corpus)

| Vite | Corpus での対応 | 状態 |
|---|---|---|
| entry / module | **UI キー** (descriptor / html / custom WC、 §11.3) | 設計済 |
| dev server の on-demand transform | Corpus が UI キーを on-demand 配信 (`/api/corpus/ui/<key>`) | 一部実装 (login で実証) |
| framework runtime (Vue/React) | **corpus-renderer** (descriptor を実描画) | 実装済 |
| route chunk の遅延ロード | feature を開いた時だけ UI キーを fetch | P1 |
| ブラウザキャッシュ / pre-bundle | UI 情報を WebStorage に version/ETag 付き保持 (§1.5) | P1 |
| **HMR** | UI キー版 bump を WS/SSE で push → shell が該当パネルだけ再描画 | P2 |
| error overlay | descriptor 検証エラーを shell にインライン表示 | P2 |
| file-watch dev server | サービスが UI をファイル著述 → Corpus watch → HMR | P3 |

### 12.2 P1 — 遅延ロード + WebStorage キャッシュ + 版管理

- UI キーは **開いた時だけ** fetch (初期 render 以外は遅延)。 §11.2 の
  initial-render は即時、 他 feature は lazy
- 取得した UI 情報 (descriptor/html) は `corpus.ui.<service>.<key>` キーで
  WebStorage に保存。 2 回目以降はそこから即描画 (§1.5)
- 配信側は UI キーごとに **version / ETag** を返す。 shell は手元の version と
  突合し、 変化が無ければ再 fetch しない (`If-None-Match` / version マップ)
- ネイティブ host は同梱で version 比較すら不要 (§1.5)

### 12.3 P2 — HMR (UI キー版 bump の push)

- Corpus が **UI キー版チャネル** (WS or SSE `/api/ui-hmr`) を開く。 サービスの
  UI キーの version が上がったら、 接続中の shell に `{service, key, version}` を
  push
- shell は該当 UI キーを描いているパネルがあれば、 **そのパネルだけ** 再 fetch +
  再描画 (全画面リロード無し)。 dock レイアウト / アクティブタブ / 入力中でない
  フォーム状態は保持を試みる (descriptor は概ね stateless なので影響小)
- version bump の検知元: discovery の定期 refresh で manifest / UI キーの version
  差分を見る (既存 discovery ループに相乗り)。 これが「サービスの UI を直すと
  即ブラウザ反映」 = Vite HMR の核
- 状態保持の限界は明記する: `custom` WC の内部状態 / 進行中フォームは
  再描画で失われ得る (Vite の HMR boundary と同じ割り切り)

### 12.4 P3 — file-watch dev mode (真の Vite dev server)

- サービスが UI を **ファイルで著述** (`*.ui.json` descriptor / `*.html` /
  custom WC ソース)。 Corpus dev mode がそれを watch
- ファイル保存 → version bump → P2 の HMR push → ブラウザ即反映
- これで「サービス開発者は UI ファイルを編集するだけ、 Corpus が dev server」
  という Vite 体験が成立。 build は Corpus 側の責務 (esbuild 相当)
- 本番は P1 の version 付き静的配信 + キャッシュに切り替わる (dev=HMR / prod=cache)

### 12.5 corpus-renderer = framework runtime / error overlay

- corpus-renderer は Vite が配る Vue/React runtime に相当。 descriptor を受けて
  実 DOM (or ネイティブ) を描く。 host 非依存 (§1.4) なので runtime 実装は
  host ごとに差せる
- descriptor 検証エラー / data 取得エラーは shell が **オーバーレイ** で出す
  (Vite の error overlay 相当)。 サイレント失敗にしない (§7 の no-silent-cap と同趣旨)

### 12.6 段階と §9 への追記

[[feedback_decision_metrics]] で評価:

| Phase | 内容 | コスト | 学習量 | 主目的一致 |
|---|---|---|---|---|
| Vite-P1 | lazy fetch + WebStorage cache + version | 低 | 中 | 高 (§1.5 を実装に) |
| Vite-P2 | HMR (版 bump WS + 部分再描画) | 中 | 高 | 高 (Vite 体験の核) |
| Vite-P3 | file-watch dev mode | 高 | 高 | 中 (開発体験の仕上げ) |

§9 移行フェーズに **Vite-P1〜P3** を P6 (frontend 撤廃) と並行可能な独立トラック
として追加する。 P2 HMR は Corpus core の WS チャネル + shell 部分再描画なので
Corpus DESIGN §13 への反映とセットで Corpus PR を立てる。

---

## 13. 統合の2軸とプレイヤー入口 (Corpus / Excubitor の役割確定)

> 2026-06-08 ユーザ確定。 プレイヤーが叩くエンドポイントをどこに置くかの設計。
> **フロント統合 = Corpus / バックエンド・インフラ統合 = Excubitor** の2軸。
> **正本は Corpus DESIGN.md / Excubitor DESIGN.md** に反映する。

### 13.1 3つの平面 (混同しない)

| 平面 | 何をする | 担う |
|---|---|---|
| **レンダラ (ライブラリ)** | descriptor / html を実描画 | `corpus-renderer` (Web shell / デスクトップ / ゲームに埋め込む、 §1.4) |
| **フロント統合 = エッジ/集約 (プレイヤー入口)** | SPA 配信・データ集約・ambient SSO セッション保持・UI キー配信・HMR | **Corpus app** |
| **バックエンド・インフラ統合 = 運用/監視 (control plane)** | catalog/scanner/control/process/log/auto_fix・インフラ統合 | **Excubitor** ([[project_excubitor]]) |

ポイント: 「Corpus = レンダラ」 ではない。 **`corpus-renderer` がレンダラ
(ライブラリ)**、 **Corpus app がそれを載せたフロント統合サーバ (エッジ + 集約)**。
プレイヤーが叩くのは Corpus *app* であって renderer *パッケージ* ではない。

### 13.2 プレイヤー入口 = Corpus app (確定)

- **フロント統合 = Corpus app**。 プレイヤーの公開エンドポイント。 web では
  SPA を配信し、 ネイティブ/ゲームでは renderer を埋め込んだクライアントが
  同じ Corpus app の集約 API (`/api/hub/*`) を叩く。 全 renderer-host の統合点
- ambient SSO の **session holder = Corpus app** (C-4 を確定。 Excubitor では
  ない)。 1 度 Corpus にログイン → 以後トークンを各サービスへ渡す
- 現状の実機もこの形: プレイヤー → Corpus(:5185) が SPA + `/auth/*` +
  `/api/hub/*` + `/ui-hmr` を担い、 裏で Cernere へサーバ間問い合わせ。
  Excubitor は経路に居ない

### 13.3 Excubitor は ops plane (入口にしない)

- **バックエンド・インフラ統合 = Excubitor**。 Corpus / 各サービスを **外から
  監視・制御**する control plane (loopback 専有)。 プレイヤーは control plane に
  触れない
- Excubitor をプレイヤー入口にすると「内部 ops」 と「公開 ingress」 が混ざり
  役割が壊れる → **入口にしない**

### 13.4 専用エッジ gateway は defer

公開 TLS/ドメイン終端を複数 Corpus/hub で共有、 web+native+マルチテナントの
単一フロントドアが必要になった時のみ、 Corpus の前に薄い edge gateway を新設
する。 それまでは Corpus app がエッジ兼集約 ([[feedback_prototyping_flow]] の
「必要が見えたら分離」)。 [[feedback_decision_metrics]] 評価で A (Corpus入口、
作業コスト0) を採用、 B (gateway新設) は defer、 C (Excubitor入口) は役割破壊で却下。

### 13.5 まとめ図

```
プレイヤー ──HTTP──▶ [ Corpus app ]  = フロント統合 (エッジ + UI 集約 + ambient SSO)
                         │  embeds corpus-renderer (描画ライブラリ)
                         ├─ /auth/*      → Cernere (認証 authority)
                         ├─ /api/hub/*   → 各サービス (データ集約、 token passthrough)
                         └─ /ui-hmr      → UI キー HMR

   [ Excubitor ] = バックエンド・インフラ統合 (監視/制御、 control plane)
        └─ Corpus / 各サービスを外から監視 (プレイヤー経路外)
```
