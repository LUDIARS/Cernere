...
通常 user / tool / MFA 待ち token の用途は明示的に分離する。旧用途なし token と
旧 Redis session は拒否し、未配線の composite MFA 完了は成功扱いしない。
現行の制約と移行条件は [トークン用途・既定値・プロフィールの境界](auth-schema-profile-boundaries.md) を参照。
