...
> **セキュリティモデル**: 公開エンドポイントは **認証 (`/auth`)** のみです。それ以外のすべての API は認証済みセキュアセッションを通じてのみアクセス可能です。データの参照・変更を含む操作は WebSocket セッション経由で実行されます（後述の「WebSocket モジュール」を参照）。

### 公開エンドポイント — 認証 (`/auth`)

認証フローの開始・コールバック処理のみ外部に公開されています。
...
以下のエンドポイントは認証済みセッションが必須です。データの変更・削除を伴う操作は WebSocket セッション経由でのみ実行されます。
