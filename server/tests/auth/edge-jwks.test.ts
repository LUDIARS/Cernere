...
describe("getEdgeSigningKey", () => {
  ...
  it("rate-limits refetches while the cache is expired and upstream keeps failing", async () => {
    const fetchImpl = vi.fn()
      .mockResolvedValueOnce(jwksResponse([jwkFor("kid-1")]))
      .mockRejectedValue(new Error("network down"));

    const expired = 7 * 60 * 60 * 1000;
    await getEdgeSigningKey(TEAM, "kid-1", 0, fetchImpl as unknown as typeof fetch);
    // 期限切れ + CF 障害中は fetchedAt が更新されない。 それでも問い合わせは
    // 1 分 1 回までに抑える (アサーション 1 通ごとに CF を叩かない)。
    for (let i = 0; i < 5; i += 1) {
      await getEdgeSigningKey(TEAM, "kid-1", expired + i * 1_000, fetchImpl as unknown as typeof fetch);
    }
